...
export const AsyncSelectInputWidget: React.FC<AsyncSelectInputWidgetProps> = ({
  ...
  return (<div className="relative">
    <button type="button" 
        disabled={disabled} 
        onClick={handleSelect} className={cn(
      ...
      {displayValue && <span className="flex-grow text-primary font-semibold text-sm ml-3 underline">{displayValue}</span>}
      {!displayValue && <span className="flex-grow text-muted-foreground text-sm ml-3">{placeholder}</span>}
