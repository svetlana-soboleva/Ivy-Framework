...
export function BladeWidget({ index, title, children, id, width, slots }: BladeWidgetProps) {
  ...
  return (
    <div style={styles} className={cn(
        "flex flex-col h-screen bg-white border-l border-gray-200",
      )}
      >
      <div className="flex items-center justify-between px-4 bg-white text-black h-[70px] border-b" onMouseDown={(e) => handleMouseDown(e)}>
        {!slots?.BladeHeader && <h2 className="text-sm font-medium">{title}</h2>}
      ...
      <div className="bg-gray-50">
        <ScrollArea type='hover' className='blade-container h-[calc(100vh-70px)]'>
          <div className='p-4'>
            {children}
          </div>
        </ScrollArea>
