...
interface AnimationWidgetProps {
  ...
  animation: 'Rotate' | 'SlideIn' | 'FadeIn' | 'ZoomIn' | 'SlideOut' | 'FadeOut' | 'ZoomOut' 
    | 'Bounce' | 'Shake' | 'Flip' | 'Stagger' | 'Wave' | 'Pulse' | 'Spring' | 'Hover',
  duration?: number,
  delay?: number,
...
const getEasing = (easing?: string) => {
  switch (easing) {
    ...
    case 'AnticipateOut': return 'anticipateOut';
...
const getAnimationVariants = (props: AnimationWidgetProps): Variants => {
  ...
  switch (animation) {
    ...
    case 'SlideIn':
      ...
      variants.exit = { ...directionOffset, opacity: 0, transition: baseTransition };
      break;
    ...
    case 'SlideOut':
      ...
      variants.animate = { ...directionOffset, opacity: 0, transition: baseTransition };
...
const AnimationWidget: React.FC<AnimationWidgetProps> = (props) => {
  ...
  const variants = getAnimationVariants({ ...rest, animation: type as AnimationWidgetProps['animation'], children });
