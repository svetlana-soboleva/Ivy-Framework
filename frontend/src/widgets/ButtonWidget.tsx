...
import { BorderRadius, getBorderRadius, getColor, getWidth } from '@/lib/styles';
...
interface ButtonWidgetProps {
  ...
  variant?: "Default" | "Inline" | "Destructive" | "Outline" | "Secondary" | "Ghost" | "Link" | "Inline"
  disabled: boolean;
  tooltip?: string;
  foreground?: string;
  loading?: boolean;
  ...
  size
    ...
    return (
      <ButtonWithTooltip
        ...
        size={buttonSize}
        onClick={handleClick}
        variant={camelCase(variant) as "default" | "destructive" | "outline" | "secondary" | "ghost" | "link" | "inline"}
        disabled={disabled}
        className={cn(
      ...
      >
        {!hasChildren && ( 
          <>
            {iconPosition == "Left" && (        
              <>
                {loading && <Loader2 className="animate-spin" style={iconStyles} />}
                {!loading && icon && icon != "None" && <Icon style={iconStyles} name={icon} />}
